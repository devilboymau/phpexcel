...
$sql2="select cve_area_resp,desc_area_resp,cve_depto_resp,desc_depto_resp,tipo_sesion,fecha_sesion,count(mes) as cta,mes
from(
...
where  (comites.borrado='0' or comites.borrado is null) and status_sesiones.asistio='1' and sesiones.num_sesion='2' and anio=".$anio.$ccondicion." order by func_direccion.cve_depto_resp,sesiones.fecha_sesion)
	as s1
...
while($array=pg_fetch_array($query2)){	
		...
		if(trim($array['tipo_sesion'])=='O' && $num_sesion=='2'){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("F".$x, $sumaE);
		$objPHPExcel->getActiveSheet()->SetCellValue("G".$x, $sumaE);
		$objPHPExcel->getActiveSheet()->SetCellValue("H".$x, $sumaO);
		$objPHPExcel->getActiveSheet()->SetCellValue("I".$x, $sumaE);
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $sumaT);
...
header('Content-Disposition: attachment;filename="Reporte Asistidas Graficas.xlsx"');

...
from(
SELECT 	func_direccion.id,trim(func_direccion.cve_area_resp) as cve_area_resp,
trim(func_direccion.cve_depto_resp) as cve_depto_resp,
...
Case 
 ...
 trim(cat_area_resp.desc_area_resp) as desc_area_resp,
 trim(cat_depto_resp.desc_depto_resp) as desc_depto_resp 
...
where  (comites.borrado='0' or comites.borrado is null) and status_sesiones.asistio='1' 
	and num_sesion='1'
	and anio=".$anio.$ccondicion." 
	order by func_direccion.cve_depto_resp,sesiones.fecha_sesion)
	as s1
...
$vins=0;
$suma=0; //SUMA TOTAL
$sumaO=0; //SUMA primera ordinaria columno
$sumaO2=0; //SUMA segunda ordinaria columna
$sumaO3=0; //SUMA tercera ordinaria columna
$sumaO4=0; //SUMA cuarta ordinaria columna
$sumaO5=0; //SUMA quinta ordinaria columna
$total=0;
$sumaT=0; //SUMA TOTAL
$depto="";
...
//variables de sumas
$suma1=0; //SUMA total de las 1 ORDINARIA fila
$suma2=0; //SUMA total de las 2 ORDINARIA fila
$suma3=0; //SUMA total de las 3 ORDINARIA fila
$suma4=0; //SUMA total de las 4 ORDINARIA fila
$suma5=0; //SUMA total de las 5 ORDINARIA fila
	while($array=pg_fetch_array($query)){	
		...
		if(trim($array['fecha_sesion'])!=$mes || trim($array['cve_area_resp'])!=$areaD  || trim($array['cve_depto_resp'])!=$depto ){
			if($no!=0){
				$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $suma1);
			}
			$no++;
		...
		$suma1=$array['cta']+$suma1;
		$sumaO=$array['cta']+$sumaO;

	/*	
		$sumaO2=$array['cta']+$sumaO2;
		$sumaO3=$array['cta']+$sumaO3;
		
		$sumaO4=$array['cta']+$sumaO4;
		$sumaO5=$array['cta']+$sumaO5;
	*/
...
$x++;
		$objPHPExcel->getActiveSheet()->getStyle('D'.$x)->applyFromArray($style);
	...
	//suma de filas
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $suma1);
		/*
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $suma2);
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $suma3);
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $suma4);
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $suma5);
		$sumaT=$suma1;
		*/
		
		
	//sumas de columnas
		$objPHPExcel->getActiveSheet()->SetCellValue("E".$x, $sumaO);
		/*
		$objPHPExcel->getActiveSheet()->SetCellValue("F".$x, $sumaO2);
		$objPHPExcel->getActiveSheet()->SetCellValue("G".$x, $sumaO3);
		$objPHPExcel->getActiveSheet()->SetCellValue("H".$x, $sumaO4);
		$objPHPExcel->getActiveSheet()->SetCellValue("I".$x, $sumaO5);
		*/
...
header('Content-Disposition: attachment;filename="Reporte Asistidas Grafica.xlsx"');

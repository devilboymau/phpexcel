...
where  (comites.borrado='0' or comites.borrado is null) 
	and status_sesiones.asistio='1'
	and num_sesion=".$i."
...
$mes="";
	while($array=pg_fetch_array($query)){	
		...
		}else if (trim($array['tipo_sesion'])=='O' ){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("G".$x, $sumaO);
		$objPHPExcel->getActiveSheet()->SetCellValue("H".$x, $sumaO);
		$objPHPExcel->getActiveSheet()->SetCellValue("I".$x, $sumaO);
		
		$objPHPExcel->getActiveSheet()->SetCellValue("J".$x, $sumaT); //SE ENCARGA DE HACER LA SUMA DE LAS FILAS

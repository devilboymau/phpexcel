...
$sql="select nombre, count(usuario) as sesion
  from usuarios 
  inner join cat_personal on usuarios.cve_personal = cat_personal.cve_personal
  inner join status_sesiones on cat_personal.cve_personal = status_sesiones.cve_personal
  inner join cat_area_resp on cat_personal.cve_areares = cat_area_resp.cve_area_resp
  where cat_area_resp.cve_area_resp='DCIGP'
  group by nombre order by sesion desc";
...
$query=pg_query($sql);
	...
	while($array=pg_fetch_array($query)){
		...
		$objPHPExcel->getActiveSheet()->SetCellValue("A".$x, $nombre=$array['nombre']);		
		$objPHPExcel->getActiveSheet()->SetCellValue("B".$x, $num_sesiones=$array['sesion']); 
		

			$data[]=array($nombre,$num_sesiones);
...
$xAxisTickValues1 = array(
	new PHPExcel_Chart_DataSeriesValues('String', 'Worksheet!$A$6:$A$15', null, 10),
...
$title1 = new PHPExcel_Chart_Title('Asistencia del Personal a Sesiones');
// INFORMACION EN EL EJE Y
$yAxisLabel1 = new PHPExcel_Chart_Title('Sesiones');
// INFORMACION EN EL EJE X
$xAxisLabel1 = new PHPExcel_Chart_Title('Personal');
...
$chart1->setTopLeftPosition('D5');
$chart1->setBottomRightPosition('P30');
...
header('Content-Disposition: attachment;filename="Reporte Sesiones.xlsx"');

...
$objPHPExcel->getActiveSheet()->getStyle('O6')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER)->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER)->setWrapText(true);
	...
	$sql1="select MAX(c) as m
	from (select count(sesiones.id) as c from sesiones 
	left join func_direccion on sesiones.id=func_direccion.id 
	left join comites on comites.id=sesiones.id 
	WHERE (borrado='0' or borrado is null) 
	and tipo_sesion='O' ".$condicion2." 
	group by sesiones.id ) t";
